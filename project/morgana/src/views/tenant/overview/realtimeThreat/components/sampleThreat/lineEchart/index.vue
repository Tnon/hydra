...
<script lang="ts">
  ...
  import { requestThreatLineData, requestSampleLineData } from '@/api/overview';
  ...
  export default defineComponent({
    ...
    setup(props) {
      ...
      const getSampleThreatData = async () => {
        // 样本请求数据
        const { fields, values } = await requestSampleLineData({
          ...props.filters,
          sample_type: props.sample_type,
        });

        // 威胁情报请求
        const { values: threatValues } = await requestThreatLineData({
          ...props.filters,
        });
