...
import { requestResetKeys } from '@/api/config';
export const serverConfig = defineComponent({
  ...
  setup(props) {
    ...
    // 获取配置和连接信息
    watchEffect(async () => {
      state.device_code = props.device_code!;
      state.link_status = props.link_status!;
      pageLoading.value = props.pageLoading;
    });
    // 重置
    const resetKeys = async () => {
      const { link_status, device_code } = await requestResetKeys();
