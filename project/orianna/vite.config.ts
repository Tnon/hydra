 ...
 * @LastEditTime: 2022-04-22 17:13:58
 */
...
export default ({ mode, command }: ConfigEnv): UserConfig => {
  ...
  const { VITE_BASE_URL, VITE_DROP_CONSOLE, VITE_PORT_ORIANNA, VITE_PROXY_ORIANNA } = viteEnv;
  ...
  return {
    ...
    server: {
      port: VITE_PORT_ORIANNA,
      host: true,
      proxy: createProxy(VITE_PROXY_ORIANNA),
      fs: {
        strict: false,
      },
    },
